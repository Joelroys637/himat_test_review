...
export default function LandingPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-color)', color: 'var(--text-primary)' }}>
      ...
      {/* Hero Section */}
      <section style={{ padding: '4rem 1rem 8rem 1rem', textAlign: 'center', position: 'relative', overflow: 'hidden', minHeight: '80vh', display: 'flex', alignItems: 'center' }}>
        <video 
          autoPlay 
          loop 
          muted 
        ...
        {/* Gradient overlay to blend video with the light theme */}
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'linear-gradient(to bottom, rgba(255,255,255,0) 0%, rgba(255,255,255,0.1) 100%)', zIndex: 0, pointerEvents: 'none' }}></div>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, maxWidth: '800px', margin: '0 auto', animation: 'slideUp 0.8s ease-out forwards' }}>
          ...
          <h1 style={{ fontSize: 'clamp(3rem, 5vw, 5rem)', fontWeight: '800', lineHeight: '1.1', marginBottom: '1.5rem', letterSpacing: '-0.02em' }}>
            ...
            </span> Reviews with AI.
          </h1>
          
          <p style={{ color: 'white', fontSize: '1.25rem', lineHeight: '1.6', marginBottom: '3rem', maxWidth: '650px', margin: '0 auto 3rem auto' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <button style={{ fontSize: '1.125rem', padding: '1rem 2rem', borderRadius: '0.75rem', background: 'rgba(0, 0, 0, 0.05)', color: 'var(--text-primary)', border: '1px solid rgba(0, 0, 0, 0.1)', cursor: 'pointer', transition: 'all 0.2s' }} onMouseOver={(e) => {e.currentTarget.style.background = 'rgba(0, 0, 0, 0.1)'; e.currentTarget.style.transform = 'scale(1.05)'}} onMouseOut={(e) => {e.currentTarget.style.background = 'rgba(0, 0, 0, 0.05)'; e.currentTarget.style.transform = 'scale(1)'}}>
              View Live Demo
            </button>
          </div>
        </div>
      ...
      <section id="features" style={{ padding: '6rem 1rem', background: '#f9fafb', borderTop: '1px solid rgba(0,0,0,0.05)', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
        <div className="container" style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '5rem' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '1.125rem', maxWidth: '600px', margin: '0 auto' }}>Everything you need to dominate local SEO without spending a dime.</p>
          </div>
          
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
            
            {/* Feature 1 */}
            <div className="glass-card" style={{ padding: '2.5rem', textAlign: 'left', background: '#ffffff', border: '1px solid rgba(99,102,241,0.2)', transition: 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)', cursor: 'default', position: 'relative', overflow: 'hidden' }} onMouseOver={(e) => e.currentTarget.style.transform = 'translateY(-10px)'} onMouseOut={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
              ...
              <div style={{ width: '60px', height: '60px', borderRadius: '16px', background: 'rgba(99,102,241,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem', border: '1px solid rgba(99,102,241,0.2)' }}>
                <QrCode size={30} color="#6366f1" />
              </div>
              <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '1rem', color: 'var(--text-primary)' }}>Custom Google QR Codes</h3>
              <p style={{ color: 'var(--text-secondary)', lineHeight: '1.6', fontSize: '1.05rem' }}>Generate stunning, print-ready QR codes wrapped in custom Google Review templates. Pick your exact brand colors to perfectly match your storefront aesthetics.</p>
            </div>

            {/* Feature 2 */}
            <div className="glass-card" style={{ padding: '2.5rem', textAlign: 'left', background: '#ffffff', border: '1px solid rgba(168,85,247,0.2)', transition: 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)', cursor: 'default', position: 'relative', overflow: 'hidden' }} onMouseOver={(e) => e.currentTarget.style.transform = 'translateY(-10px)'} onMouseOut={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
              ...
              <div style={{ width: '60px', height: '60px', borderRadius: '16px', background: 'rgba(168,85,247,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem', border: '1px solid rgba(168,85,247,0.2)' }}>
                <LayoutDashboard size={30} color="#a855f7" />
              </div>
              <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '1rem', color: 'var(--text-primary)' }}>Dedicated Vendor Portal</h3>
              <p style={{ color: 'var(--text-secondary)', lineHeight: '1.6', fontSize: '1.05rem' }}>Manage your entire operation from a sleek, centralized dashboard. Update your business keywords, track analytics, and regenerate QRs instantly.</p>
            </div>

            {/* Feature 3 */}
            <div className="glass-card" style={{ padding: '2.5rem', textAlign: 'left', background: '#ffffff', border: '1px solid rgba(34,197,94,0.2)', transition: 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)', cursor: 'default', position: 'relative', overflow: 'hidden' }} onMouseOver={(e) => e.currentTarget.style.transform = 'translateY(-10px)'} onMouseOut={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
              ...
              <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '1rem', color: 'var(--text-primary)' }}>100% Free. No Limits.</h3>
              <p style={{ color: 'var(--text-secondary)', lineHeight: '1.6', fontSize: '1.05rem' }}>No hidden subscriptions, no "per-scan" fees, and no feature gating. RateXpeed is entirely free to use for unlimited scans and unlimited review generations.</p>
            </div>
      ...
      <section id="how-it-works" style={{ padding: '7rem 1rem' }}>
        <div className="container" style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '5rem' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '1.125rem', maxWidth: '600px', margin: '0 auto' }}>Three simple steps to build an unshakeable online reputation.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '4rem', position: 'relative' }}>
            {[
              ...
              <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', position: 'relative', transition: 'transform 0.3s', cursor: 'default' }} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
                <div style={{ width: '90px', height: '90px', borderRadius: '50%', background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '2rem', position: 'relative', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.1)' }}>
                  {item.icon}
                  <div style={{ position: 'absolute', top: '-5px', right: '-5px', background: '#ffffff', color: 'var(--text-primary)', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.875rem', fontWeight: 'bold', border: '2px solid #e5e7eb' }}>
                    {item.step}
                  </div>
                </div>
                <h3 style={{ fontSize: '1.35rem', fontWeight: '600', marginBottom: '1rem', color: 'var(--text-primary)' }}>{item.title}</h3>
                <p style={{ color: 'var(--text-secondary)', lineHeight: '1.6', fontSize: '1rem' }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA Section */}
      <section style={{ padding: '7rem 1rem', background: 'linear-gradient(to right, rgba(99,102,241,0.08), rgba(168,85,247,0.08))', textAlign: 'center', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
        <div className="container" style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.25rem', marginBottom: '3rem' }}>Join hundreds of vendors who are automating their 5-star Google reviews and boosting their SEO, 100% free.</p>
